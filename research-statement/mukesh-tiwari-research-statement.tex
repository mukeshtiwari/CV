...
\item focus on formally verified combinators for algebraic structure (CAS) in Coq theorem prover 
	...
	\emph{semiring} --set $A$ equipped with two binary operators $+$ and $*$, 
	known as (abstract) addition and (abstract) multiplication, $0$, (abstract) additive identity, 
	$1$, (abstract) multiplicative identity-- as an underlying structure
	...
	can compute shortest path in a graph, longest paths in a graph, analyse the 
	data flow of imperative programs, optimally pack knapsacks, and perform 
	discrete event simulations \cite{gondran2008graphs, 10.1145/2500365.2500613}.
